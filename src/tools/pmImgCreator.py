...
import string, sys, types, dis, os, time, getopt, struct, types
...
################################################################
    ...
    def _seq_to_str(self, seq):
        ...
        for i in range(len(seq)):
            ...
            # if it is None
            elif objtype == type(None):
                # marker, none (0)
                imgstr += _U8_to_str(OBJ_TYPE_NON)

            # if it is a float
            elif objtype == float and PM_FEATURES["HAVE_FLOAT"]:
                imgstr += _U8_to_str(OBJ_TYPE_FLT) + self._float_to_str(obj)

            # other type?
            else:
                raise NotImplementedError("Unhandled type %s." % objtype)
        return imgstr
...
################################################################
    ...
    def _filter_co(self, co):
        ...
        ## Bcode filter
        # bcode string
        s = "".join(map(chr, list(co.co_code)))
        ...
        if (len(consts) > 0 and type(consts[0]) == str):
